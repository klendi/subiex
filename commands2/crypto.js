...
Subiex.registerCommand(
  ...
  (message, bot) => {
    //  Init variables
    let symbols = message.content.split(' ').map(function(x) {
  ...
  ['crypt', 'price'],
  'Get latest crypto exchange price in USD, or in other cryptos.',
  '<crypto-currency ticker> (crypto-currency ticker )'
)
...
function CrytpoComparePrice(fsym, tsyms, message) {
  ...
  request(cryptoComparePrice, function(error, response, body) {
    ...
    let finalMessage =
      '**~~------~~** __Current exchange rates for 1 ' +
      fsym +
      '__ **~~------~~**\n```'
    try {
